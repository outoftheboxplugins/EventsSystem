...
void FEventsSystemEditorToolkit::Initialize(UEventsSystem* InEventsSystem, const EToolkitMode::Type InMode, const TSharedPtr<class IToolkitHost>& InToolkitHost)
{
	EventsSystem = InEventsSystem;

	// Support undo/redo
	EventsSystem->SetFlags(RF_Transactional);
...
{
	return FString(TEXT("https://github.com/ue4plugins/EventsSystem"));
}
...
{
	Collector.AddReferencedObject(EventsSystem);
}
...
TSharedRef<SDockTab> FEventsSystemEditorToolkit::HandleTabManagerSpawnTab(const FSpawnTabArgs& Args, FName TabIdentifier)
{
	...
	{
		TabWidget = SNew(SEventsSystemEditor, EventsSystem, Style);
	}
